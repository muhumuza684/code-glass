...
Continuous low-volume motorbike-style engine bed with state-driven cues and deduplication.
...
Run `apply.ps1` from PowerShell with the Code Glass project path configured. The installer copies this package under `addons/codeglass/08-adaptive-audio` and creates an integration note. Source-level wiring is described in `INTEGRATION.md`.
