...
Canonical project-state adapter that preserves legacy stages and exposes three processes.
...
Run `apply.ps1` from PowerShell with the Code Glass project path configured. The installer copies this package under `addons/codeglass/01-state-adapter` and creates an integration note. Source-level wiring is described in `INTEGRATION.md`.
