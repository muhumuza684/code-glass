...
Compatibility layer mapping process actions to legacy stage mutation safely.
...
Run `apply.ps1` from PowerShell with the Code Glass project path configured. The installer copies this package under `addons/codeglass/04-write-path-bridge` and creates an integration note. Source-level wiring is described in `INTEGRATION.md`.
