...
Pure migration helpers and tests for legacy five-stage manifests.
...
Run `apply.ps1` from PowerShell with the Code Glass project path configured. The installer copies this package under `addons/codeglass/02-manifest-migration` and creates an integration note. Source-level wiring is described in `INTEGRATION.md`.
