...
Three-process domain model with internal checkpoints and failure propagation.
...
Run `apply.ps1` from PowerShell with the Code Glass project path configured. The installer copies this package under `addons/codeglass/03-process-core` and creates an integration note. Source-level wiring is described in `INTEGRATION.md`.
